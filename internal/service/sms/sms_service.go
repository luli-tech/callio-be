...
type Service struct {
	...
	gateway        domain.SMSGateway
	cfg            *config.BillingConfig
	logger         *logger.Logger
	httpClient     *http.Client
	workerWg       sync.WaitGroup
	callbackChan   chan *domain.SMSMessage
	closeOnce      sync.Once
	closed         chan struct{}
}
...
func NewService(
	...
	s := &Service{
		...
		callbackChan: make(chan *domain.SMSMessage, 500),
...
func (s *Service) Send(ctx context.Context, req *domain.SendSMSRequest) (*domain.SMSMessage, error) {
	...
	if strings.TrimSpace(req.Body) == "" {
		return nil, domain.ErrInvalidSMSBody
	}
	...
	if err != nil {
		...
		msg.Status = domain.SMSStatusFailed
		msg.ErrorCode = "30001"
	...
	if msg.Status == domain.SMSStatusFailed {
...
func (s *Service) GetBySID(ctx context.Context, accountID, sid string) (*domain.SMSMessage, error) {
...
func (s *Service) HandleDeliveryReport(ctx context.Context, carrierMsgID string, status domain.SMSStatus, errCode, errMsg string) error {
